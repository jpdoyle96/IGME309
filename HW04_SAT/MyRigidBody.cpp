...
uint MyRigidBody::SAT(MyRigidBody* const a_pOther)
{
	...
	// Put rotation matrix for other in coordinate frame of this
	for (uint i = 0; i < 3; i++)
	{
		for (uint j = 0; j < 3; j++)
		{
	...
	// arithmetic errors when two edges are parallel
	for (uint i = 0; i < 3; i++)
	{
		for (uint j = 0; j < 3; j++)
		{
