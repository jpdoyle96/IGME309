...
void MyMesh::GenerateCircle(float a_fRadius, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	*/
	// Sets up the local variables needed to store points
	vector3 zero = vector3(0.0f, 0.0f, 0.0f);
	std::vector<vector3> outer_vertices;
	GLfloat theta = 0;
	GLfloat delta = static_cast<GLfloat>(2.0 * PI / static_cast<GLfloat>(a_nSubdivisions));
	// Calculates the radial points and adds them to outer_vertices list
	for (int i = 0; i < a_nSubdivisions; i++)
	{
		vector3 vertex = vector3(cos(theta) * a_fRadius, sin(theta) * a_fRadius, 0.0f);
		theta += delta;
		outer_vertices.push_back(vertex);
	}
	for (int i = 0; i < a_nSubdivisions; i++)
	{
		AddTri(zero, outer_vertices[i], outer_vertices[(i + 1) % a_nSubdivisions]);
	}
